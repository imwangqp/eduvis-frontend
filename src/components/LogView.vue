  ...
  watch(
      ...
      (newValue, oldValue) => {
        ...
        if(newValue > oldValue) {
          ...
          axios.post('/api/getAnswerLog', {'id': cards[newId]}).then(res => {
            if(res.data.code) {
...
function initLineChart(index) {
  ...
  var line = d3.line()
      .x((d) => x(d.x))
      .y(d => {/*console.log("yyyyyyyyyyyyy"+y(d.y));*/
        return yy(d.y)
      })
      .curve(d3.curveBasis);
  ...
  //平均折线图
  var line2 = d3.line()
      .x((d, i) => {/*console.log("xxxx"+i); */
        return x(i)
      })
      .y(d => {/*console.log("yyyy"+y(d));*/
        return y(d)
      })
      .curve(d3.curveBasis);
